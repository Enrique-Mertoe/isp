...
def gen_mtk_provision(request):
    if request.method == 'POST':
        ...
        mtk_info = {
            "name": f"{user.username}{(count + 1101)}",
...
def provision_content(request, encoded_payload):
    ...
    provisioning_url = f"{server_url}/provision_content/{encoded_payload}"
...
def provision_version_content(request, encoded_payload, version):
    try:
        ...
        config = {
            ...
            "vpn_url":f"{settings.API_URL}/mikrotik/openvpn/{router.identity}/{router.secrete}"
        }
